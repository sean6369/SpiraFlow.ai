...
import { saveEntry, saveAudio, getAllEntries, getUserPatterns, saveUserPatterns } from '@/lib/db';
import { analyzeUserPatterns } from '@/lib/analytics';
import { JournalEntry, AIPrompt } from '@/types';
...
export default function HomePage() {
    ...
    const generateLivePrompts = async (transcription: string, wordCount: number) => {
        try {
            ...
            // Use the dedicated live prompts endpoint for shorter, immediate questions
            const response = await axios.post('/api/generate-live-prompts', {
                transcription,
            }, {
                timeout: 6000, // 6 second timeout for faster response
            });

            const prompts = response.data.prompts.map((p: any, idx: number) => ({
                id: `live-prompt-${Date.now()}-${idx}`,
                prompt: p.prompt,
                context: p.context,
                timestamp: new Date(),
                category: 'live', // Mark as live prompts for styling
            }));
    ...
    const performLiveTranscription = async () => {
        try {
            ...
            const response = await axios.post('/api/transcribe-live', formData, {
    ...
    const stopRecording = async () => {
        try {
            ...
            while (retryCount <= maxRetries) {
                try {
                    transcribeResponse = await axios.post('/api/transcribe', formData, {
                        headers: { 'Content-Type': 'multipart/form-data' },
            ...
            const { transcription: text } = transcribeResponse.data;
            ...
            try {
                const titleResponse = await axios.post('/api/generate-title', {
                    transcription: text,
                });
                title = titleResponse.data.title || 'Untitled Entry';
            } catch (error) {
            ...
            const promptsResponse = await axios.post('/api/generate-prompts', {
                currentEntry: tempEntry,
                recentEntries: allEntries.slice(-5),
                userPatterns: patterns,
            });

            const prompts = promptsResponse.data.prompts.map((p: any, idx: number) => ({
